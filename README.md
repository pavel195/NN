...
- Python 3.8+
- 8 GB RAM
- 10 GB свободного места на диске (для датасета и моделей)

**Рекомендуемые требования:**
- Python 3.10+
- 16 GB RAM
- GPU с поддержкой CUDA (для ускорения обучения)
- 20 GB свободного места на диске

**Время обучения:**
- На CPU: ~8-10 часов для 50 эпох
- На GPU: ~2 часа для 50 эпох
...
### Проблема: "Директория data/lfw/... не найдена"

**Решение**: Убедитесь, что датасет LFW скачан и распакован в правильную директорию:
```bash
# Проверьте структуру
ls data/lfw/lfw-deepfunneled/lfw-deepfunneled/ | head -5
```
...
```bash
python train.py --batch_size 16  # или даже 8
```
...
- Увеличьте `num_workers` в DataLoader (если достаточно RAM)
- Уменьшите количество эпох для тестирования

## Дополнительная информация

Подробное описание работы, экспериментов и результатов представлено в файле `coursework_report.md`.
...
При возникновении вопросов обращайтесь к авторам работы.
